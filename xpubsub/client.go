...
type Interface interface {
	...
	PullMessage(ctx context.Context, subsName string) ([]byte, error)
...
func (c *PubSubClient) PublishMessage(ctx context.Context, topicID string, data []byte) error {
	close, err := c.getClient(ctx)
	if err != nil {
		return err
	}
...
func (c *PubSubClient) PullMessage(ctx context.Context, subsName string) ([]byte, error) {
	close, err := c.getClient(ctx)
	if err != nil {
		return nil, err
	}
